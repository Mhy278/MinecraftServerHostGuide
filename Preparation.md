...
##### 什么是 IPC ？
> IPC（Inter-Process Communication），即进程间通信，指 CPU 每一时钟周期内所执行指令的多少。

IPC 代表了一款处理器的设计性能，一旦该处理器设计完成之后，IPC 值就不会再改变了。
...
同时，无论纯净与模组服务器，单个核心的负载能力都是有限的，如果你的玩家人数较多，我们建议你采用跨服的方式，讲玩家分别进入多个子服务端，这样可以令多个线程同时工作，以达到负载均衡的目的。
...
**注意：若按照上述顺序操作过程中，出现任何错误，请利用上方“自身条件”中第4条的能力排查错误原因，排除错误。
